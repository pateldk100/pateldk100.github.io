<html>
<title>Library Management System | Delete Deactivated User</title><body>
<head>
<style type="text/css">
<!--
.tbl {
...
function fun_delete_user(){
		if(confirm("Going to delete record........")==1)
        document.user_list_form.action="delete_user_process.php";
}
</script>
</head>
...
$wcond="and ((active_deactive=0 and active_deactive_date is not null) ";
...
if(isset($_POST['txtsrch']))
	$wcond=$wcond." and (r.name like '". trim($_POST['txtsrch'])."%' or r.design = '". trim($_POST['txtsrch'])."' 
					or r.class = '". trim($_POST['txtsrch'])."')";	
...
<div class="h">Library Info System | Delete User</div>
...
<div align="center" style="background:#F4F4F4; color:Red; padding:2px;	width:100%; height:335px; overflow:auto;">
    <table width="100%" border="0" cellspacing="0" cellpadding="5" align="left" border="0">
         <tr align="left"><strong>
			...
			<th width="15%" class="tbl">Design./Class</th>
...
$i=0;	
	$sql="SELECT r.lib_reg_no, r.IDSR, r.name, r.design, r.class, r.sec, r.f_name, u.usertype, u.login_date 
			from reader_mast r, user_mast u  where r.lib_reg_no=u.lib_reg_no and r.lib_reg_no <>'".$_SESSION['id']."' " . $wcond;
...
?>	
		<tr bgcolor= <?php if($i%2==0) echo '#C5E8E1'; else echo '#E2EFD0'; ?>  >
			<td width="5%"> <input TYPE="checkbox" NAME="lib_reg_no[]" value="<?php echo $lib_reg_no ?>" ></td>
			<td width="8%"><?php echo $lib_reg_no ?>
			<td width="25%"> <?php echo $name ?> </td>
			<td width="15%"> <?php echo " [ ".$design.$class." ".$sec." ]" ?> </td>
...
?>		
	...
	<table width="90%" border="0">	
		<tr>
			<td width="50%" align="center"><input type="submit" name="delete" value="Delete" onclick="fun_delete_user()"/></td>
			<td width="50%" align="center"><input type='submit' name='cancel' value="Cancel" onclick="fun_cancel()"></td>
	</table>
<hr>	
</form>	
