...
<?php //if($_SESSION['utype']=='A') $openPHP="lib_reg_list.php";  else $openPHP="account_panel.php"; 
	if(isset($_POST['back']))
		$back=$_POST['back'];
	else
		$back='account_panel.php';
...
<form name="user_entry" action="signup_process.php?action=update"  onsubmit="return validate_user_entry_form()" method="POST" > 
...
<?php 
  ...
  <?php
		$sql="SELECT distinct class FROM class_mast";
		$rs=odbc_exec($conn,$sql);
		while(odbc_fetch_row($rs))
			{ 	$cl=odbc_result($rs,"class");
			
				if($v_clas==$cl) $pst= $cl."' selected"; else $pst=$cl."'";
